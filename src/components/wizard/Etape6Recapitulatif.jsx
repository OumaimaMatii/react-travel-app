/**
 * Etape6Recapitulatif.jsx – Récapitulatif complet + calcul prix + soumission.
 *
 * Calcul :
 *   prix_hotel     = Σ (prix_par_nuit × quantite × nb_nuits)
 *   prix_activites = Σ (prix_adulte × nb_adultes + prix_adulte×0.5 × nb_enfants)
 *   prix_transport = (prix_aller + prix_retour) × (nb_adultes + nb_enfants)
 *   commission     = sous_total × commission% / 100
 *   total          = sous_total + commission
 */
...
export default function Etape6Recapitulatif() {
  ...
  useEffect(() => { dispatch(fetchCommission()) }, [dispatch])
  ...
  return (
    <div className="space-y-5">
      ...
      <Section icon={<MapPin size={16} className="text-secondary" />} title="Voyage">
        ...
        <Row label="Date départ"  value={formatDate(wizard.dateDepart)} />
        <Row label="Date retour"  value={formatDate(wizard.dateRetour)} />
        ...
        <Section icon={<Hotel size={16} className="text-secondary" />} title="Hébergement">
          ...
          {wizard.chambres.map((c, i) => (
            <Row
              key={i}
              label={c.nom}
              value={`${c.quantite} × ${c.prix_par_nuit} DH/nuit × ${nbNuits} nuits = ${formatPrice(c.prix_par_nuit * c.quantite * nbNuits)}`}
            />
          ))}
          <Row label="Sous-total hôtel" value={formatPrice(prixHotel)} bold />
        </Section>
      )}
          ...
          {wizard.activites.map((a, i) => {
            ...
            return (
              <Row
                key={i}
                label={a.nom}
                value={`${a.nb_adultes}A${a.nb_enfants > 0 ? ` + ${a.nb_enfants}E` : ''} = ${formatPrice(st)}`}
              />
            )
          })}
          <Row label="Sous-total activités" value={formatPrice(prixActivites)} bold />
        </Section>
      )}
      ...
      <Section icon={<Plane size={16} className="text-secondary" />} title="Transport">
        {wizard.transportAller ? (
          <Row
            ...
            value={`${wizard.transportAller.prix} DH × ${totalPersonnes} = ${formatPrice(prixTranspAller)}`}
          />
        ) : (
        ...
        {wizard.transportRetour && (
          <Row
            ...
            value={`${wizard.transportRetour.prix} DH × ${totalPersonnes} = ${formatPrice(prixTranspRet)}`}
          />
        )}
        <Row label="Sous-total transport" value={formatPrice(prixTransport)} bold />
      </Section>
      ...
      <div className="bg-primary/5 border border-primary/10 rounded-xl p-4 space-y-2">
        ...
        <div className="flex justify-between text-sm">
          ...
          <span className="font-medium">{formatPrice(montantComm)}</span>
        </div>
