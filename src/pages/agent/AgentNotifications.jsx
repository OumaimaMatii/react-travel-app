    ...
    <div className="space-y-6 max-w-2xl">
      ...
      {loading ? <Loader /> : items.length === 0 ? (
        ...
        <div className="space-y-2">
          {items.map(n => (
            <div key={n.id}
              className={`card p-4 flex items-start gap-4 ${!n.lue ? 'border-secondary/30 bg-secondary/5' : ''}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <p className={`text-sm font-semibold ${!n.lue ? 'text-primary' : 'text-gray-700'}`}>{n.titre}</p>
                  {!n.lue && <span className="w-2 h-2 bg-secondary rounded-full flex-shrink-0 mt-1.5" />}
                </div>
                <p className="text-sm text-gray-500 mt-0.5 leading-relaxed whitespace-pre-line">{n.message}</p>
